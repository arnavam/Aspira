...
def compress_subgraph_sources(subgraph: dict) -> dict:
    ...
    # Create Proxy Node
    proxy_id = "proxy_web_resources"  # Unique per subgraph visualization since they are isolated files
    proxy_node = {
        "id": proxy_id,
        "type": "proxy",
    ...
    new_nodes.extend(source_nodes) # Keep sources in the list so they are still visible when expanding connected to proxy
...
def create_pyvis_network(graph_data: dict, output_path: str):
    ...
    net = Network(height=net_height, width="100%", bgcolor="#0b0c10", font_color="#c5c6c7", directed=True)
    ...
    type_config = {
        ...
        "proxy": {"color": "#e74c3c", "icon": "f07b", "size": 45, "label_color": "#ffffff"} # Folder icon
    }
    
    seen_ids = set()
    for node in graph_data["nodes"]:
        n_id = node["id"]
        if n_id in seen_ids: continue
        seen_ids.add(n_id)
        ...
        net.add_node(
            ...
            font={'color': config['label_color'], 'face': 'arial', 'size': 16, 'strokeWidth': 2, 'strokeColor': "#000000"}
        )
    ...
    for edge in graph_data["edges"]:
        ...
        if relation in ["required_for", "enables"]:
            color = "#66fcf1"; width = 3; font_color = "#66fcf1"
        elif relation in ["subset_of", "part_of"]:
            color = "#45a29e"; width = 2; dashes = True; font_color = "#45a29e"
        elif relation == "source_for":
            color = "#2c3e50"; width = 1
        elif relation == "references_resources":
            color = "#e74c3c"; width = 2; font_color = "#e74c3c"
        elif relation == "contains":
            color = "#555555"; width = 1; dashes = True
        ...
        net.add_edge(
            ...
            dashes=dashes,
            font={'color': font_color, 'size': 12, 'align': 'middle', 'background': '#0b0c10', 'strokeWidth': 0},
...
def create_master_visualization(subgraphs: List[dict], output_base_name: str, output_dir: str):
    ...
    for i, subgraph in enumerate(subgraphs):
        ...
        print(f"  - Generated subgraph {i+1} ({len(compressed_subgraph['nodes'])} nodes)")
...
def main():
    if len(sys.argv) > 1:
        ...
        if files:
            files.sort(key=lambda f: int(f.replace("log/knowledge_map_", "").replace(".json", "")))
            filepath = files[-1]
        else:
    ...
    output_base = "knowledge_graph_" + os.path.basename(filepath).replace(".json", "")
    
    master_path = create_master_visualization(subgraphs, output_base, output_dir)
