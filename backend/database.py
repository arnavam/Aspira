...
class Database:
    ...
    def _init_indexes(self):
        ...
        self.db.keywords.create_index([("user_id", 1), ("keyword", 1)], unique=True)
        
        # QA Cache: user_id + question unique
        self.db.qa_cache.create_index([("user_id", 1), ("question", 1)], unique=True)
        
        # Conversation: index on user_id for fast retrieval
        self.db.conversation.create_index([("user_id", 1), ("id", 1)])
    ...
    def get_session_counter(self, user_id: str) -> int:
        doc = self.db.meta.find_one({"user_id": user_id, "key": "session_counter"})
    ...
    # --- KEYWORDS (KW) ---
    def get_keywords(self, user_id: str) -> dict:
    ...
    def update_keywords(self, user_id: str, kw_dict: dict):
        ...
        try:
            self.db.keywords.delete_many({"user_id": user_id})
            if kw_dict:
                self.db.keywords.insert_many([
                    {"user_id": user_id, "keyword": k, "scores": v, "updated_at": datetime.utcnow()} 
    ...
    def add_conversation_message(self, user_id: str, message: str):
    ...
    def get_conversation_history(self, user_id: str) -> list:
        cursor = self.db.conversation.find({"user_id": user_id}).sort("timestamp", 1)
        return [doc["message"] for doc in cursor]
